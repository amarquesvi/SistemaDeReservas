...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (senhaValida) {
                // Usuário válido, obter o objeto Usuariopelo email
                Usuario usuario = mU.obterUsuarioPorEmail(email);
                
                 //Obtém a sessão atual
                HttpSession session = request.getSession();
                 // Armazena o obj usuario na sessão com a chave "usuario"
                session.setAttribute("usuario", usuario);
                // Armazena a ID do usuario na sessão com a chave "id"
            ...
            request.setAttribute("erroLogin", "Erro ao validar o login ");
            request.getRequestDispatcher("paginaLogin.jsp").forward(request, response);
